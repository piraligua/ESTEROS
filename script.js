document.addEventListener("DOMContentLoaded", () => {
  ...
  const reserveCartItemsList = document.querySelector(
    ".reserve-cart-items-list"
  );
  ...
  const closeReservePopupButton = document.querySelector(
    ".close-reserve-popup"
  );
  ...
  function renderCart() {
    ...
    cart.forEach((item) => {
      ...
      li.innerHTML = `<img src="${item.img}" alt="${item.name}" width="50"> ${
        item.name
      } x${item.quantity} - ${itemTotal.toFixed(
        3
  ...
  function updateReserveCartItems() {
    ...
    cart.forEach((item) => {
      ...
      li.innerHTML = `<img src="${item.img}" alt="${item.name}" width="50"> ${
        item.name
      } x${item.quantity} - $${(item.price * item.quantity).toFixed(3)}`;
